...
teamiddf = 
tibble(
id = ESPNFromJSON2$teams$id,
location = ESPNFromJSON2$teams$location,
name = ESPNFromJSON2$teams$nickname
) %>%
  mutate(team = paste(location,name)) %>%
  select(id,team)
return(teamiddf)
}
...
weeklyplayerperformancedata = weeklyplayerperformance()
weeklyplayerperformance = function(weeks = 3, leagueID = "89417258",year = "2019"){
...
getsteamnamesandrecords()
getsteamnamesandrecords = function(weeks = weeks, year = 2019, leagueID = leagueID){
  PlayerTeamDF = NULL
